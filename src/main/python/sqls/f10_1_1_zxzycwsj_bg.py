...
(
select
     case when stkcode.TRADE_MKT = '深圳证券交易所' then 'SZ' when stkcode.TRADE_MKT = '上海证券交易所' then 'SH' else '' end + stkfin.b_stockcode as Obj
...
--   , stkfin.RPT_YEAR, stkfin.RPT_TAG
   ...
   , stkfin.EPSNED as MeiGuShouYi--每股收益(元)-每股收益EPS-最新股本摊薄
   , stkfin.EPSNED * case stkfin.RPT_TAG when 1 then 4.0 when 5 then 2.0 when 6 then 1.3333 when 7 then 1.0 else 0 end as MeiGuShouYiJiSuan--每股收益EPS-最新股本摊薄-乘上系数，不适合展示，只用于计算
...
--   , 0 as ZhuYingShouRuTongBi   --主营收入同比
   ...
   , stkshr.B_SHR / 10000 as LiuTongGu   --当前流通股(A股或B股)(万股)------需要增加(大智慧无)
...
--   , stkshr.FL_SHR / 10000 as WuXianShouGuHeJi   --无限售股合计(万股)
   ...
   , stkshr.B_SHR / 10000 / stkholder.TOT_HLD as PingJunGuBen   --平均股本(万股)
...
inner join STK_CODE stkcode WITH(NOLOCK) on stkfin.B_STOCKCODE = stkcode.STOCKCODE and stkcode.ISVALID = 1
...
left join STK_SHR_STRU stkshr with(nolock) on stkfin.B_STOCKCODE = stkshr.B_STOCKCODE and stkshr.ISVALID = 1
                       and stkshr.CHANGEDATE = (select max(CHANGEDATE) from STK_SHR_STRU with(nolock) where B_STOCKCODE = stkshr.B_STOCKCODE and ISVALID = 1)
...
left join STK_LIST_RESULT stklist with(nolock) on stklist.ISVALID = 1 and stklistplan.SEQ=stklist.P_SEQ and stkcode.COMCODE = stklist.COMCODE and stklist.LIST_DATE is not null
                              --and stkfin.B_STOCKCODE = '900942'

where stkfin.ISVALID = 1 and stkfin.B_STOCKCODE is not null and stklist.LIST_DATE is not null
