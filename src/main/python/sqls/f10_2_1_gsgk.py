...
sql = """SELECT CASE X.TRADE_MKT_REF
       ...
       indu.INDU_NAME AS sshy, A.WEB_SITE AS gswz,
       A.EMAIL AS dzxx, X.LIST_DATE AS ssrq,
       QKTZ20160429.dbo.Date2Str( X.LIST_DATE ) AS II_ssrq, G.INTRO_DCL_DATE AS zgrq,
       QKTZ20160429.dbo.Date2Str( G.INTRO_DCL_DATE ) AS II_zgrq, F.NEW_SHR AS fxl,
       F.ISS_PRC AS fxj, E.TOPEN AS srkpj,
  ...
  LEFT JOIN PUB_INDU_REF AS indu WITH ( NOLOCK )
         ON B.CSRC_INDU_CODE_2012 = indu.INDU_CODE
        AND indu.ISVALID = 1
        AND indu.INDU_SYS_MARK = 14
