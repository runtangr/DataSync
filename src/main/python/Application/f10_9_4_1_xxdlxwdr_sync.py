...
class GetDataFromMssql(SyncSys):
  ...
  def GetArgs(self,DBObj):
      ...
      last_update_time, last_rsid = self.get_last_update_info(DBObj=DBObj,action_name="f10_9_4_1_xxdlxwdr", key_value="all")
  ...
  def GetData(self, action_name = "f10_9_4_1_xxdlxwdr", parse_args=None):
          try:
...
class SaveDataToDB(SyncSys):
  ...
  def Save(self,MssqlData):
    ...
    Colection = self.db.f10_9_4_1_xxdlxwdr.find().count()
    AllColection = self.db.collection_names()

    # map(lambda data:self.db.f10_9_4_1_xxdlxwdr.insert(data) , mssql_data)
    map(lambda data: self.SaveData(data), mssql_data)
    # 写入sync时间
    last_data = mssql_data[-1]

    self.db.f10_0_1_update_timestamp.update({"Name":"f10_9_4_1_xxdlxwdr"},
  ...
  def SaveData(self, data):
      ...
      while True:
          try:
            count = self.db.f10_9_4_1_xxdlxwdr.find({"RsId":data["RsId"]}).count()
            break
      ...
      if count > 0:
        ...
        try:
            ...
            self.db.f10_9_4_1_xxdlxwdr.update({"RsId":data["RsId"]},{"$set":data})
        except pymongo.errors.AutoReconnect, e:
            logging.error('AutoReconnect fail\n')
            time.sleep(2)
      else:
        ...
        try:
            ...
            self.db.f10_9_4_1_xxdlxwdr.insert(data)
        except pymongo.errors.AutoReconnect, e:
            logging.error('AutoReconnect fail\n')
            time.sleep(2)
